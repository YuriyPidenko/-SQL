...
CREATE or replace VIEW v_friends AS 
  ...
  DELEMITER // 
  CREATE FUNCTION delete_user_from_vk(user_id INT) RETURNS INT 
  BEGIN
  ...
  SELECT delete_user_from_vk(1); -- в скобках id пользователя
...
BEGIN
	IF CHAR_LENGTH(NEW.name) < 5 THEN 
		SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = "Название сообшества";
	END IF;
END//
  
DELIMITER ;   
 
INSERT INTO  `communities` (name) VALUES ('12345');